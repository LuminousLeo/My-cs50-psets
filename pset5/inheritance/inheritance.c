...
person *create_family(int generations)
{
    ...
    if (generations > 1)
    {
        ...
        //get first allele from the first parent randomly
        int i = rand() % 2;
        current->alleles[0] = parent0->alleles[i];

        //get second allele from second parent randomly
        int n = rand() % 2;
        current->alleles[1] = parent1->alleles[n];
